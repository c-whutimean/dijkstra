#Compile the code
# if the file densemult edits then remove it
n=9
cmd=graph
hwnumber=5
if [ -f ${cmd} ]; then
...
for casenum in `seq 1 1 ${n}`; do
	timeout -k 2s 2s ./$cmd "input=inputT${casenum}.txt;path=pathT${casenum}.txt;output=outputT${casenum}.txt" 1>T${casenum}.stdcout 2>T${casenum}.stderr
